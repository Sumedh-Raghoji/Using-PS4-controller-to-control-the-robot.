...
class SimpleTfKinematics(Node):
    def __init__(self):
        ...
        self.static_transform_stamped_.header.stamp = self.get_clock().now().to_msg() #self.get_clock() is inherited from node class
        ...
        self.get_logger().info("Publishing static transform betweem %s and %s" %
                               (self.static_transform_stamped_.header.frame_id, self.static_transform_stamped_.child_frame_id))
        
        self.timer_ = self.create_timer(0.1, self.timeCallback)
